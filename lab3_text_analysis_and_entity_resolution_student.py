...
def tokenize(string):
    ...
    """
    filter_func = lambda s: s not in stopwords
    return filter(filter_func, simpleTokenize(string))
...
def tfidf(tokens, idfs):
    ...
    """
    tfs = tf(tokens)
    tfIdfDict = {k:vtf * idfs[k] for k, vtf in tfs.iteritems() if k in idfs}
    return tfIdfDict
...
def dotprod(a, b):
    ...
    """
    return sum([v * b[k] for k, v in a.iteritems() if k in b])
...
def norm(a):
    ...
    """
    return math.sqrt(sum([v ** 2 for k, v in a.iteritems()]))
...
amazonWeightsRDD = (amazonFullRecToToken
                    .map(lambda (k, v): (k, tfidf(v, idfsFullWeights))))
...
def invert(record):
    ...
    """
    pairs = [[k, record[0]] for k, v in record[1].iteritems()]
    return (pairs)
...
# TODO: Replace <FILL IN> with appropriate code
amazonWeightsBroadcast = sc.broadcast(amazonWeightsRDD)
googleWeightsBroadcast = sc.broadcast(googleWeightsRDD)
...
def fastCosineSimilarity(record):
    ...
    tokens = record[1]
    s = sum([amazonWeightsBroadcast.value[amazonRec][t] * googleWeightsBroadcast.value[amazonRec][t] for t in tokens])
    value = (s / norm(googleNormsBroadcast.value[amazonRec])) / norm(amazonNormsBroadcast.value[googleRec]) 
