...
def getCountsAndAverages(IDandRatingsTuple):
    ...
    rateLst = IDandRatingsTuple[1]
    CountsAndAverages = (movieID, (len(rateLst), sum(rateLst) * 1.0 / len(rateLst)))
...
def computeError(predictedRDD, actualRDD):
    ...
    squaredErrorsRDD = (predictedReformattedRDD
                        ...
                        .map(lambda (k, v): (k, math.pow(v[0] - v[1], 2))))
    
    # Compute the total squared error - do not use collect()
    totalError = squaredErrorsRDD.map(lambda s: s[1]).sum()
...
trainingAvgRating = trainingRDD.map(lambda s: s[2]).sum() / trainingRDD.count()
...
myUnratedMoviesRDD = (moviesRDD
                      .filter(lambda s: s[0] not in [i[1] for i in myRatedMovies])
                      .map(lambda (k, v): (myUserID, k)))
...
movieCountsRDD = movieIDsWithAvgRatingsRDD.map(lambda (k, v): (k, v[0]))

# Transform predictedRatingsRDD into an RDD with entries that are pairs of the form (Movie ID, Predicted Rating)
predictedRDD = predictedRatingsRDD.map(lambda s: (s.product, s.rating))
...
ratingsWithNamesRDD = (predictedWithCountsRDD
                       .join(moviesRDD)
                       .filter(lambda (m, ((p, n), t)): n > 75)
                       .map(lambda (m, ((p, n), t)): (p, t, n)))
